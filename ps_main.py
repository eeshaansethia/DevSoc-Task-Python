...
def get_oldest(data):
    ...
    for i in data["AVENGERS"]:
        if (data["AVENGERS"][i]['age'] > max):
            ...
            oldest_marvel = data["AVENGERS"][i]
    oldest.append(oldest_marvel)
...
def get_oldest_avenger(data):
    ...
    return oldest_marvel


# returns info: Thor
print(get_oldest_avenger(data_json))
...
print(get_total_points(data_json))
...
def get_more_than_average(data):
    ...
    for j in data["DC"]:
        if (data["DC"][j]['points']['strength'] > avg_dc):
            more_than_average.append(data["DC"][j])
    '''
...
print(get_more_than_average(data_json))
